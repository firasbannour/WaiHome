...
export default function DetailsHighWater() {
  const navigation = useNavigation();
  const route = useRoute();
  
  // Récupérer les états depuis SiteDetails
  const { siteId, highWaterState, onHighWaterChange } = route.params as any;
  
  const [highWaterOn, setHighWaterOn] = useState(highWaterState || false);
  const [confirmVisible, setConfirmVisible] = useState(false);
  const [pendingValue, setPendingValue] = useState(false);
  const anim = React.useRef(new Animated.Value(highWaterOn ? 1 : 0)).current;

  // Synchroniser avec les états de SiteDetails
  useEffect(() => {
    setHighWaterOn(highWaterState || false);
  }, [highWaterState]);

  React.useEffect(() => {
    Animated.timing(anim, {
      toValue: highWaterOn ? 1 : 0,
      duration: 220,
  ...
  const handleSwitchPress = () => {
    setPendingValue(!highWaterOn);
    setConfirmVisible(true);
  };
  
  const handleConfirm = async (ok: boolean) => {
    if (ok) {
      const newHighWaterState = pendingValue;
      setHighWaterOn(newHighWaterState);
      
      // Notifier SiteDetails du changement
      if (onHighWaterChange) {
        onHighWaterChange(newHighWaterState);
      }
      
      // NOUVEAU : Contrôler la sortie physique Shelly (OUTPUT-3 pour High Water)
      const success = await controlShellyComponent(siteId, 'highWater', newHighWaterState);
      if (success) {
        console.log('✅ Shelly High Water output toggled successfully');
      } else {
        console.error('❌ Error toggling Shelly High Water output');
      }
      
      // Sauvegarder dans AWS DynamoDB (mise à jour, pas création)
      try {
        console.log('🔄 Saving high water change to AWS...');
        ...
        if (currentUserId) {
          ...
          if (sitesJson) {
            ...
            if (currentSite && currentSite.deviceInfo) {
              // Payload minimal de mise à jour
              const updates = {
                components: {
                  pump: currentSite.deviceInfo.components?.pump || false,
                  auger: currentSite.deviceInfo.components?.auger || false,
                  heater: currentSite.deviceInfo.components?.heater || false,
                  highWaterAlarm: newHighWaterState,
                  emergency: currentSite.deviceInfo.components?.emergency || false,
                  binReplaced: currentSite.deviceInfo.components?.binReplaced || false,
                  heaterStarted: currentSite.deviceInfo.components?.heaterStarted || false,
                },
              ...
              if (result.success) {
                console.log('✅ High water state saved to AWS successfully');
              } else {
      ...
      console.log('🔧 High Water state changed to:', newHighWaterState);
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.title}>High Water</Text>
        </View>
      ...
      <View style={styles.statusCard}>
        <View style={styles.statusRow}>
          ...
          <View>
            <Text style={styles.statusBold}>High Water Alarm Triggered</Text>
      ...
      <View style={styles.manualCardRow}>
        ...
        <View style={{ flex: 1, marginLeft: 16 }}>
          <Text style={styles.statusBold}>High Water</Text>
          <Text style={styles.statusDate}>
            Status: <Text style={highWaterOn ? styles.badgeOn : styles.badgeOff}>{highWaterOn ? 'On' : 'Off'}</Text>
          </Text>
        </View>
        {/* Switch custom animé */}
        <TouchableOpacity onPress={handleSwitchPress} activeOpacity={0.8} style={{ marginLeft: 10 }}>
          <View style={[styles.switchTrack, { backgroundColor: highWaterOn ? '#16989d' : '#b0b0b0' }] }>
      ...
      <Modal visible={confirmVisible} transparent animationType="fade" onRequestClose={() => handleConfirm(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <MaterialCommunityIcons name="water" size={36} color="#16989d" style={{ marginBottom: 12 }} />
            <Text style={{ fontSize: 18, fontWeight: '700', marginBottom: 18, color: '#16989d', textAlign: 'center' }}>
              {pendingValue ? 'Turn ON the high water alarm?' : 'Turn OFF the high water alarm?'}
            </Text>
            {!pendingValue && (
